...
const UserDashboard = () => {
  ...
  useEffect(() => {
    const loadDashboard = async () => {
      try {
        const response = await axiosInstance.get(TASKS.userDashboard)
  ...
  return (
    <DashboardLayout title="My Dashboard" subtitle="Track your assigned tasks, deadlines, and completion progress.">
      <div className="space-y-8">
          ...
          {['Total tasks', 'Pending tasks', 'Completed tasks', 'Overdue tasks'].map((label, index) => (
            <div key={label} className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <p className="mt-4 text-3xl font-semibold text-slate-900">
                {loading ? '...' : [summary?.totalTasks, summary?.pendingTasks, summary?.completedTasks, summary?.overdueTasks][index] ?? 0}
              </p>
            </div>
          ))}
        </div>
        ...
        <div className="grid gap-6 xl:grid-cols-2">
          <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <h3 className="text-lg font-semibold text-slate-900">My priority breakdown</h3>
          ...
          <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <h3 className="text-lg font-semibold text-slate-900">Task status summary</h3>
            <div className="mt-6 h-72">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
